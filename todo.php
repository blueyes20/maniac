    ...
    <div id="tabs">
			<?php
				...
				$envio1=mysqli_query($con,$num);
					...
					echo'<ul>';
					while($prioridades=mysqli_fetch_array($envio1)){
					
						echo'<li><a href="#tabs-'.$prioridades["id_prioridad"].'">'.$prioridades["nombre_prioridad"].'</a></li>';
					
					}
					echo'</ul>';
					
					while($prioridades){
						echo'<div id="tabs-'.$prioridades["id_prioridad"].'"><p>'.$prioridades["id_prioridad"].'</p>
						 </div>';
